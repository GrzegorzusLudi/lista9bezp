...
void prime(mpz_t cel,int keysize){
    ...
    mpz_t p,rand,pdiv;mpz_init(cel);mpz_init(p);mpz_init(rand);mpz_init(pdiv);
    char* cs;
    do{
        int ks = keysize;
        /*do{
        ...
        while(ks>4){
            //cout<<ks<<endl;
            ks=ks/2;
            cs = generaterandom(ks);
            mpz_init_set_str(rand,cs, 16);
            mpz_mul(cel,cel,rand);
        }
        //gmp_printf ("%Zx\n", cel);
        mpz_mul_ui(cel,cel,2);
        mpz_add_ui(cel,cel,1);
    }while(!mpz_probab_prime_p(cel,33));
    //gmp_sprintf (ret,"%Zx\n", p);
}
string passwd(){
